...
fn main() {
    ...
    for entry in entries {
        ...
        writeln!(
            output,
            "        TEST_CASES.get_or_init(|| load_test_cases(\"{}\"))",
            file_name
        )
        .unwrap();
        writeln!(output, "    }}").unwrap();
        ...
        writeln!(
            output,
            "            \"Test index {{}} out of bounds (total: {{}})\","
        )
        .unwrap();
        ...
        writeln!(
            output,
            "        let value = convert_json_value(&test_case.value, Some(test_case.expected.as_str()));"
        )
        .unwrap();
        writeln!(output).unwrap();
        writeln!(
            output,
            "        let result = if let Some(ref opts) = test_case.options {{"
        )
        .unwrap();
        writeln!(
            output,
            "            let options = parse_options(Some(opts));"
        )
        .unwrap();
        writeln!(
            output,
            "            format_with_options(&test_case.pattern, value, options)"
        )
        .unwrap();
        ...
        writeln!(
            output,
            "                    \"Test case {{}}: pattern='{{}}', value={{:?}}, options={{:?}}\","
        )
        .unwrap();
        writeln!(
            output,
            "                    index, test_case.pattern, test_case.value, test_case.options"
        )
        .unwrap();
        writeln!(output, "                );").unwrap();
        writeln!(output, "            }}").unwrap();
        ...
        writeln!(
            output,
            "                    index, test_case.pattern, test_case.value, test_case.options, e"
        )
        .unwrap();
        writeln!(output, "                );").unwrap();
        writeln!(output, "            }}").unwrap();
